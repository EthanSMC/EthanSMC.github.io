...
当前版本还显式等待真实微信发表列表的完整就绪/可穷尽契约验收：加载中、部分结果、分页、无法识别页面和未接入该契约的真实页面都会失败关闭，DOM 零匹配不会授权发布或撤回。即使将私密开关误改为 `1`，这道门也会在自动点击前停止。撤回后若已有公开微信 URL，还必须验证该精确 HTTPS URL 返回确定的不可用状态；仍可读取或响应不明会保留待核对状态并终止本轮后续点击。

撤回标记的 `requestedAt` 是规范 UTC 代次。Agent 会先记录已消费代次，因此文章恢复后保留的旧标记不会让以后一次普通删除变成撤回授权；只有再次把同 ID 文章精确移回私密草稿、生成严格更新的代次，才产生新授权。
